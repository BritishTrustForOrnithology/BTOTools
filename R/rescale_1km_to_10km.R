...
#'
#' @description
#' Takes a vector of British National grid 1-km references and converts to 10-km references.
#' Works for Britain, Ireland and Channel Isles grids
#'
#' @param x character vector containing the input grid reference
#'
#' @return A character vector with tenkm references
#' 
#' @examples
#' temp1<-data.frame(cbc_code='R.',count=10,onekm='TL1234', stringsAsFactors = FALSE)
#' rescale_1km_to_10km_(temp1$onekm)
#'
#' @export
#'
rescale_1km_to_10km_<-function(x) {
  #check input parameters
  if(!is.character(x)) stop('invar must be supplied as a character string')
  ...
  tenkm<-paste(substr(x,1,3),substr(x,5,5),sep=''  )
  return(tenkm)
}
